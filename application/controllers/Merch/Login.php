<?php
class Merch_LoginController extends Merch_CommonController {
	public function init() {
		parent::init();
	}
	public function indexAction(){
        if($this->isLogin){
            header("Location:/Merch_index/index");
            exit;
        }
    ...
    private function dologin(){
        ...
        $res = $Business->setUserCookie($info['mid']);
        if( $res === false ){
    ...
    public function loginOutAction(){
        $this->delCookieVal();
        header("Location:/Merch_login/index" );
    }
}