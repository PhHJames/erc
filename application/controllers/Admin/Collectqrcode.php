...
class Admin_CollectqrcodeController extends Admin_BaseAuthController {
    ...
    //预估消耗的eth
    function estimateEthAction(){
        $params = $this->getParams();
        $Business = Common::ImportBusiness("CollectQrcode" ,"Admin" );
        try{
            $data = $Business->estimateEth( $params );
            $eth = isset($data['_eth']) ? (string) $data['_eth'] : '' ;
            Common::EchoResult(1 ,  "操作成功" , ['eth' => $eth]   );
        }catch(Exception $e ){
            Common::EchoResult($e->getCode() , $e->getMessage());
        }
    }
    ...
    public function beatch_trans_trxAction(){
        ...
        if($finish == 1 ){
            echo "<h1>批量转入ETH数据已经执行完毕！</h1>";
            exit;
            $this->showError("系统提示" , "批量转入ETH数据已经执行完毕！");
        }
        if( empty($ar_arr) ){
    ...
    public function beatch_trans_out_trxAction(){
        ...
        $Business = Common::ImportBusiness("CollectQrcode" ,"Admin" );
        try{
            $Business->Out_trx($address , $qr_id , $amout,1 );
        }catch(Exception $e ){
            Log_Db::WriteLog("beatch_trans_out_eth" , "批量ETH转出出错了,二维码id：{$qr_id}：".$e->getMessage() );
            echo "<h1>ETH转出出错了：" . $e->getMessage() . "</h1>";
        }
        $str = implode("," ,$ar_arr ) ;
        $finish = (empty($ar_arr)) ? 1 : 0 ;
        $this->echo_now("正在执行批量转出ETH数据 请等待,请不要关闭浏览器");
    ...
    public function beatch_trans_usdtAction(){
        ...
        try{
            ...
            Log_Db::WriteLog("beatch_trans_usdt" , "用户钱包USDT转出出错了,二维码id：{$qr_id}：".$e->getMessage() );
