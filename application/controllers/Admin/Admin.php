 ...
 * @Last 不要复制我的代码不然后果自负
 * @Last Modified time: 2018-11-10 12:10:34
 */
class Admin_AdminController extends Admin_BaseAuthController {
    ...
    public function userAction(){
        ...
        $this->displayTemplate('admin/user' , $res);
    }
    private function getAjaxUserList(){
        ...
        $data = $Business->getAdminUserList($params,$pageSize);
        ...
        if( !empty($list) ){
            foreach ($list as $key => $value) {
                ...
                if( $role_ids ){
                    ...
                    $roleData = $RoleModel->getBeatchRoleByRoleId($role_ids);
                    if( !empty($roleData)){
                        foreach ($roleData as $skey => $svalue) {
                            $role_str .= "<button class='layui-btn layui-btn-sm' >".$svalue['role_name']."</button>"   ;
                        }
                    }
                }
                $list[$key]['role_str'] = $role_str ;
            }
        }
    ...
    public function userEditAction(){
        ...
        $res = array(
            ...
            'info' => $info,
            'role_ids'=>$role_ids,
            'jsonRole' => json_encode(Source_Role::getRoleNav($menu,0,$user_id))
        );
    ...
    public function addRoleAction(){
        ...
        $res = array(
            'menu' => $menu ,
            'jsonRole' => json_encode(Source_Role::getRoleNav($menu))
        );
    ...
    public function editRoleAction(){
        ...
        $res = array(
            ...
            'jsonRole' => json_encode(Source_Role::getRoleNav($menu,$roleid) , JSON_UNESCAPED_UNICODE)
        );
