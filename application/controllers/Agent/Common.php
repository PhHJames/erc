<?php
//后台的基类控制器
class Agent_CommonController extends Yaf_Controller_Abstract {
    public $userInfo = array();
    public $isLogin = false ;
    public $current_url = '';
    public $cookie_name = "agent_admin_auth";
    public $agent_id = 0 ;
    ...
    public function initLogin(){
        ...
        if($authData['agent_id'] > 0 ){
            $this->isLogin = true ;
            $this->userInfo = $authData;
            $this->agent_id = $this->userInfo['agent_id'];
        }
    }
    //获取cookie的值
    ...
    public function displayTemplate($file = null , $args = array()){
        ...
        $_G['viewObject']->display( "agent/" . $file.".htm");
    }
    //显示错误页面,并且根据时间进行跳转
    ...
    */
     ...
     *  写入代理商的操作日志
     */
    public function writeActionLog(){
        $agent_id  =  $this->agent_id  ;
        $username =  isset($this->userInfo['account']) ? $this->userInfo['account'] : "" ;
        $dispatcher = Yaf_Dispatcher::getInstance();
        $request = $dispatcher->getRequest() ;
        $url = strtolower($request->controller)."/".strtolower($request->action);
        ...
        $AgentActionLogModel = new AgentActionLogModel();
        $AgentActionLogModel->insertData($res);
    }
    public function checkPlat(){
        $is_close_plat = Source_Sysconfig::getInstance()->getVal("is_close_plat");
        if($is_close_plat == 'Y' ){
            $message = Source_Sysconfig::getInstance()->getVal("plat_close_message");
            if( empty($message )){
                $message = '平台维护中 请稍后' ;
            }
            $this->showError("系统提示" , $message);
        }
    }
}